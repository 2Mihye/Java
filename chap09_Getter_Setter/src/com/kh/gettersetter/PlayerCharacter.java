...
public class PlayerCharacter {
	...
	/*
	 getter : 데이터의 복사본을 던져주기 때문에 원래 객체의 데이터는 손상시키지 않음. 원본 데이터는 안전함.
	 */
	...
	/*
	 setter : 외부에서 메서드를 통해 데이터에 접근을 하게 된다면 메서드는 매개값(=매개변수값)을 검증해서 유요한 값만 데이터로 저장할 수 있음.
	 */
	...
	public void setHealth(int health) {
		// 만약 체력이 0이라면 죽이고 체력이 1 이상이면 살리기
	...
	public void attack(PlayerCharacter target) {
		System.out.println("[" + name + "]는 공격해서 " + "[" + target.getName() + "]에게 " + attackPower + " 데미지를 줬습니다.");
